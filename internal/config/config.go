...
func load(envPath string, validateRequired bool) (Config, error) {
	...
	cfg := Config{
		...
		AI: AIConfig{
			...
			Fallback:              fallbackAI,
			MaxOutputTokens:       getInt(values, "AI_MAX_OUTPUT_TOKENS", 2048),
...
func fallbackAIConfig(values map[string]string, primaryProvider, geminiAPIKey, geminiModel string) *AIConfig {
	...
	return &AIConfig{
		...
		Model:                 model,
		GeminiModel:           geminiModel,
		MaxOutputTokens:       getInt(values, "AI_FALLBACK_MAX_OUTPUT_TOKENS", getInt(values, "AI_MAX_OUTPUT_TOKENS", 2048)),
		GeminiThinkingLevel:   get(values, "GEMINI_THINKING_LEVEL", "high"),
